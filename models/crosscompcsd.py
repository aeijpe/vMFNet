...
class CrossCSD(nn.Module):
    def __init__(self, args, device, image_channels, num_classes, vMF_kappa):
        ...
        self.rec_module_source = CompCSDRec(device, image_channels, args.layer, args.vc_num, vMF_kappa, self.norm)
        self.rec_module_target = CompCSDRec(device, image_channels, args.layer, args.vc_num, vMF_kappa, self.norm)
        self.segmentor = Segmentor(num_classes, args.vc_num_seg, args.layer)

        if self.content_disc:
            self.discriminator = DiscriminatorC(input_channels=args.vc_num_seg).to(device)
...
class CrossCSD2(nn.Module):
    def __init__(self, args, device, image_channels, num_classes, vMF_kappa):
        ...
        if self.content_disc:
            self.discriminator = DiscriminatorC(input_channels=args.vc_num_seg)
            self.optimizer_disc = optim.Adam(self.discriminator.parameters(), lr=args.learning_rate*0.1)
            self.scheduler_disc = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer_disc, 'max', patience=args.k2)
            self.gen_loss = GeneratorLoss()
            self.disc_loss = DiscriminatorLoss()
        ...
        self.cluster_loss_s = ClusterLoss()
        self.dice_loss = DiceLoss(softmax=True)
        self.gen_loss_s = GeneratorLoss()

        self.l1_distance_t = nn.L1Loss().to(device)
        self.cluster_loss_t = ClusterLoss()
        self.gen_loss_t = GeneratorLoss()
    ...
    def forward_source(self, img_s):
        rec_s, features_s, kernels_s, norm_vmf_activations_s = self.rec_module_source(img_s)
        fake_img_t = self.rec_module_target.get_image(norm_vmf_activations_s)
        fake_norm_vmf_activations_t, _, features_fake_t = self.rec_module_target.get_activations(fake_img_t)
        cross_img_s = self.rec_module_source.get_image(fake_norm_vmf_activations_t)

        results = {"rec_s": rec_s, "features_s": features_s, "kernels_s": kernels_s, "norm_vmf_activations_s": norm_vmf_activations_s,
                    "fake_norm_vmf_activations_t": fake_norm_vmf_activations_t, "cross_img_s": cross_img_s}
        return results

    def forward_target(self, img_t):
        rec_t, features_t, kernels_t, norm_vmf_activations_t = self.rec_module_target(img_t)
        fake_img_s = self.rec_module_source.get_image(norm_vmf_activations_t)
        fake_norm_vmf_activations_s, _, features_fake_s  = self.rec_module_source.get_activations(fake_img_s)
        cross_img_t = self.rec_module_target.get_image(fake_norm_vmf_activations_s)

        results = {"rec_t": rec_t, "features_t": features_t, "kernels_t": kernels_t, "norm_vmf_activations_t": norm_vmf_activations_t,
                    "fake_norm_vmf_activations_s": fake_norm_vmf_activations_s, "cross_img_t": cross_img_t}
        return results


    def update_normal(self, img_s, label_s, img_t):
        results_s = self.forward_source(img_s)
        results_t = self.forward_target(img_t)
        pre_seg_s = self.segmentor(results_s["norm_vmf_activations_s"])

        # Discriminators
        prob_true_source_is_true = self.discriminator_source(img_s)
        prob_fake_source_is_true = self.discriminator_source(results_s["cross_img_s"])
        prob_true_target_is_true = self.discriminator_target(img_t)
        prob_fake_target_is_true = self.discriminator_target(results_t["cross_img_t"])
        
        reco_loss_s = self.l1_distance_s(results_s["rec_s"], img_s)
        cross_reco_loss_s = self.l1_distance_s(results_s["cross_img_s"], img_s)

        # if epoch = 5 --> start optimizing kernels??
        clu_loss_s = self.cluster_loss_s(results_s["features_s"].detach(), results_s["kernels_s"]) 
        fake_clu_loss_s = self.cluster_loss_s(results_t["features_fake_s"].detach(), results_s["kernels_s"])
    ...
    def update(self, x_s, m_s, x_t):
        if self.content_disc:
            batch_loss_it = self.update_with_disc(x_s, m_s, x_t)
            return batch_loss_it
        else:
            batch_loss_it = self.update_normal(x_s, m_s, x_t)
            return batch_loss_it